...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
  try {
    ...
    $db->db_connect();
    $req = 'SELECT * FROM accounts.service';
    if (isset($_GET['admin'])) {
      $req .= ' where id > 0';
    } else {
      $req  .= " where sl_full_name = '" . $_GET['sl_full_name'] . "'";
    }
    echo $req;
    ...
    //   echo json_encode(['token' => $jwt, 'role' => $result['role'], 'expires_at' =>  $expires_at, 'user_email' => $username]);
    //   closeConn();
    // } else {
